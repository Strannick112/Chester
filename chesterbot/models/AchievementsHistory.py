...
class AchievementsHistory(Base):
    ...
    def __repr__(self) -> str:
        return f"AchievementsHistory(wipe_id={str(self.wipe_id)!r}, discord_account_id={str(self.discord_account_id)!r}, score={str(self.score)!r})"

    @staticmethod
    async def get_or_create(session, wipe_id, discord_account_id, **kwargs):
        instance = (await session.execute(select(AchievementsHistory).filter_by(
            **kwargs, wipe_id=wipe_id, discord_account_id=discord_account_id)
        )).scalars().first()
        if instance:
            pass
        else:
            instance = AchievementsHistory(**kwargs, wipe_id=wipe_id, discord_account_id=discord_account_id)
