...
import chester_bot.bot_commands.wipe_manage.utils
from .utils import make_claim, mark_claim_executed
from ... import replies, wipes, main_config
                    ...
                    if msg.author.__str__() == user_name:
                        ...
                        if has_reactions['claim_full_approved']:
                            if claim.status not in (Status.approved, Status.executed):
                                msg.remove_reaction(replies['claim_full_approved'], self.bot.user)
                        else:
                        ...
                        if has_reactions['claim_items_executed']:
                            if claim.status != Status.executed:
                                await msg.remove_reaction(replies['claim_items_executed'], self.bot.user)
                        else:
                            if claim.status == Status.executed:
